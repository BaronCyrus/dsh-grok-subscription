 ...
 * When no realpath matches, fall back to the only profile declaring it.
 */
...
/**
 * Owns the plugin's version self-knowledge. Registry responses and CLI output
 * stay host-only; only the small owned projection crosses RPC.
 */
export function createGrokPluginManager({
  ...
  const fetchLatest = async signal => {
    ...
    if (parseSemver(version) === undefined) throw new Error('Grok plugin version check failed')
    return version
  }

  const readOwnVersion = async () => {
    const version = (await readManifest(fileURLToPath(ownPackageJsonUrl)))?.version
    if (parseSemver(version) === undefined) throw new Error('Grok plugin version is unavailable')
    return version
  }
  ...
  return Object.freeze({
    async read({ force = false, signal } = {}) {
      ...
      return clone(await (signal === undefined ? pending : Promise.race([
        pending,
