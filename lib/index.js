...
function createSessionService({
  ...
  const storeToken = async (token) => {
    ...
    const timeoutMs = typeof storeTokenTimeoutMs === "number" && storeTokenTimeoutMs > 0 ? storeTokenTimeoutMs : STORE_TOKEN_TIMEOUT_MS;
    try {
      await withTimeout(
      ...
      throw error instanceof Error ? error : new Error("Failed to store Grok credential");
    }
  };
