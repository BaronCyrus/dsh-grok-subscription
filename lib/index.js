...
var GROK_API_HOSTS = Object.freeze(["cli-chat-proxy.grok.com"]);
var HTTP_PROXY_PATTERN = /^https?:\/\//iu;
var HOST_PROXY_PACKAGE = "@deepseek-ai/dsh-http-proxy";
var installed;
var dependencies = { undici: void 0, hostProxies: void 0 };
...
async function ensureApiRouting(env = process.env) {
  ...
  try {
    ...
    const previous = typeof getGlobalDispatcher === "function" ? getGlobalDispatcher() : void 0;
    setGlobalDispatcher(routing);
    installed = {
      url,
      dispatcher: routing,
      restore: () => {
        if (previous !== void 0) setGlobalDispatcher(previous);
        installed = void 0;
      }
    };
    return "plugin";
...
async function findInstall({ dshHome, ownPackageJsonUrl }) {
  ...
  for (const entry of entries) {
    ...
    try {
      const installed2 = await realpath(join5(profileDir, "node_modules", PACKAGE_NAME, "package.json"));
      if (installed2 === ownReal) return { kind: classifySpec(spec), profile: entry.name };
    } catch {
    }
  }
